...
#+BEGIN_SRC emacs-lisp
 ...
 (add-hook 'c++-mode-hook
           (function (lambda ()
                       ...
                       (local-set-key (kbd "<f4>") 'stampede-compile-unit-tests)
                       (local-set-key (kbd "<f5>") 'stampede-compile-debug)
                       (local-set-key (kbd "<f6>") 'stampede-compile-balanced)
                       (local-set-key (kbd "<f3>") 'stampede-compile-func-interactive)
                       (local-set-key (kbd "<f2>") 'stampede-clean-all)
                       (toggle-truncate-lines 1)
                       (setq indent-tabs-mode t)
                       (setq defun-block-intro 0)
...
#+BEGIN_SRC emacs-lisp
 (defun stampede-premake-func ()
   (format "cd %s; cd config; ./premake5 gmake2" (projectile-project-root)))

 (defun stampede-compile-func (proj config cores)
   "Generic build function for stampede"
   (interactive)
   (setq compile-command (format "%s; cd %s; cd config; cd build; cd gmake2; make %s config=%s -j%s"
                                 (stampede-premake-func) (projectile-project-root) proj config cores))
   (compile compile-command))

 (defun stampede-compile-func-interactive (proj config cores)
 ...
 (defun stampede-compile-balanced ()
   ...
   (interactive)
   (stampede-compile-func "Stampede" "balanced" "11"))

 (defun stampede-compile-unit-tests ()
   "Compiles unit tests."
   (interactive)
   (stampede-compile-func "StampedeUnitTests" "balanced" "11"))

 (defun stampede-compile-debug ()
   "Compiles unit tests."
   (interactive)
   (stampede-compile-func "Stampede" "debug" "11"))
